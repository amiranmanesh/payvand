...
flowchart TB
    subgraph app["Your application"]
        code["code written against payvand.Gateway"]
    end

    subgraph facade["package payvand — facade"]
        init["Init / New / MustNew"]
        names["gateway name constants"]
        alias["type aliases + shared options"]
    end

    subgraph core["package core — contracts"]
        iface["Gateway interface"]
        dto["Purchase / Verify / Refund / Inquiry DTOs"]
        money["Money, Currency"]
        reg["registry"]
        errs["errors + capabilities"]
    end

    subgraph gws["package gateway/* — one per provider"]
        rest["REST: zarinpal, zibal, vandar, idpay, payir, nextpay,
        payping, bitpay, yekpay, payweb, saman, sepehr,
        asanpardakht, top"]
    ...
    subgraph internal["package internal/* — plumbing"]
...
Each gateway package holds three files, always the same shape:

| File | Contents |
|---|---|
| `<name>.go` | the `Gateway` implementation, endpoints, response-code mapping |
| `dto.go` | the wire structs of that provider, unexported |
| `options.go` | the provider's opt-in options, returned as `payvand.Option` |
